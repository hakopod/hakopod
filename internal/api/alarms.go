...
func applicationAlarmObservations(app store.Application, current cluster.Observation, observationErr error, at time.Time) []store.AlarmObservation {
	...
	for _, name := range spec.Names(app.Spec) {
		...
		if observationErr == nil && found {
			...
			if (service.Status == "ready" || service.Status == "sleeping" && service.Desired == 0 || service.Status == "scheduled" && app.Spec.Services[name].Job != nil && app.Spec.Services[name].Job.Schedule != nil || service.Status == "stopped" && app.Spec.Services[name].Suspended && service.Desired == 0 || service.Status == "completed" && app.Spec.Services[name].Job != nil) && service.Ready >= service.Desired {
				health = "healthy"
				ready++
				...
				if app.Spec.Services[name].Job != nil {
					...
					if app.Spec.Services[name].Job.Schedule != nil {
						message = service.Message
					}
				}
