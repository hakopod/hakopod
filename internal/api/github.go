...
func (s *Server) planSource(w http.ResponseWriter, r *http.Request) {
	...
	write(w, 200, map[string]any{"application_id": a.ID, "expected_revision": a.Revision, "spec": next, "changes": spec.Diff(&a.Spec, next), "warnings": spec.Warnings(next), "commit_sha": commit, "expected_source_revision": b.Revision})
}
