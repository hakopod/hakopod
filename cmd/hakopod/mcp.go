...
func serveAgent(ctx context.Context, c *client, cfg config, allowDeploy bool, in io.Reader, out io.Writer) error {
	...
	server := agentServer{client: c, cfg: cfg, allowDeploy: allowDeploy, plans: map[string]agentPlan{}}
...
func (s *agentServer) application(ctx context.Context, id string) (store.Application, error) {
	var a store.Application
	if !agentID.MatchString(id) {
		return a, errors.New("invalid application ID")
	}
	if err := s.client.request(ctx, "GET", "/applications/"+id, nil, "", &a); err != nil {
		return a, err
	}
...
func (s *agentServer) call(ctx context.Context, name string, raw json.RawMessage) (any, error) {
	switch name {
	...
	case "applications":
		...
		var out any
		err := s.client.request(ctx, "GET", "/applications?"+url.Values{"project": {s.cfg.Project}, "environment": {s.cfg.Environment}, "cursor": {in.Cursor}, "limit": {"25"}}.Encode(), nil, "", &out)
		return out, err
	case "application", "deployment":
		...
		if name == "application" {
			return s.application(ctx, in.ID)
		}
		var out store.Deployment
		if err := s.client.request(ctx, "GET", "/deployments/"+in.ID, nil, "", &out); err != nil {
			return nil, err
		}
	...
	case "logs":
		...
		var out any
		err = s.client.request(ctx, "POST", "/applications/"+a.ID+"/logs/query", map[string]any{"service": in.Service, "query": in.Query, "limit": 100, "since_seconds": 3600}, "", &out)
		return out, err
	case "plan":
		var in struct {
		...
		if err := s.client.request(ctx, "POST", "/plan", map[string]any{"project": s.cfg.Project, "environment": s.cfg.Environment, "toml": in.TOML}, "", &plan); err != nil {
			return nil, err
		}
	...
	case "deploy":
		...
		var out store.Deployment
		err := s.client.request(ctx, "POST", "/deployments", plan.Body, plan.Key, &out)
		return out, err
	default:
