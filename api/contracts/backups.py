...
schemas['BackupSource'] = obj({'kind': {'type':'string','enum':['database','management']}, 'application_id':S, 'service':S, 'engine':{'type':'string','enum':['postgresql','mysql','clickhouse']}, 'database':S}, ['kind','engine'])
...
object_key = {'type':'string','description':'Destination key of the encrypted object for a logical dump. For a backup the database engine performed itself this is the prefix the engine wrote a tree of objects under, not a single object.'}
sha256 = {'type':'string','description':'Hex SHA-256 the server computed over the encrypted object. Empty, and no longer required, for a backup the database engine performed itself: those bytes never passed through the server, so there is nothing for it to digest.'}
artifact_format = {'type':'string','description':"Archive format the server recorded. A value beginning 'engine:' means the database engine wrote the backup to object storage itself, so the artifact has no digest and its object key is a prefix."}
schemas['BackupArtifact'] = obj({'id':S,'job_id':S,'destination_id':S,'source':ref('BackupSource'),'object_key':object_key,'sha256':sha256,'bytes':I,'format':artifact_format,'scope':S,'schedule_id':S,'created_at':T,'deleted_at':T},['id','job_id','destination_id','source','object_key','bytes','format','scope','created_at'])
