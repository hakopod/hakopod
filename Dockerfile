...
ARG TARGETOS=linux
ARG TARGETARCH
RUN --mount=type=cache,target=/root/.cache/go-build CGO_ENABLED=0 GOOS=${TARGETOS} GOARCH=${TARGETARCH} GOMAXPROCS=2 go build -p 2 -trimpath -ldflags='-s -w' -o /out/hakopod-server ./cmd/hakopod-server
