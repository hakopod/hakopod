...
3. **Runtime:** choose image defaults or override the command and arguments, configure variables and secrets (including paste/import `.env` with automatic secret separation), and choose resources and public HTTP access. Use **Add secret** for other confidential values. Registry credentials default to automatic matching; custom credentials remain available. Existing application builds preserve their resource and networking settings.
