...
runtime_root = Path('/opt/hakopod-actions-fixture')
subprocess.run(['docker', 'exec', node, 'sh', '-c', '! command -v runsc'], check=True)
subprocess.run(['docker', 'exec', node, 'mkdir', '-p', str(runtime_root)], check=True)
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    for name in ('runsc', 'containerd-shim-runsc-v1', 'gvisor-bin'):
        ...
        subprocess.run(['docker', 'cp', str(matches[0]), f'{node}:{runtime_root}/{name}'], check=True)
