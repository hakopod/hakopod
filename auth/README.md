...
`StartRuntime` is optional and starts the same bounded API/worker lifecycle as the engine command. It requires a real managed-cloud Kubernetes connection with exactly one registered node. The returned management handler must be placed behind product authorization; never expose it directly to ordinary Cloud customers. Installation and host administration remain restricted in managed-cloud mode.
